      ...
      <Dialog open={isOpen && step === 1} onOpenChange={(open) => !open && handleClose()}>
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <Input id="customer-name" value={name} onChange={handleNameChange} placeholder="e.g., John Doe" />
          </div>
          <DialogFooter>
      ...
      <Dialog open={isOpen && step === 2} onOpenChange={(open) => !open && handleClose()}>
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <Input id="customer-phone" type="tel" value={phone} onChange={handlePhoneChange} placeholder="03001234567" maxLength={11} />
            <p className="text-xs text-muted-foreground pt-1">Please enter an 11-digit phone number.</p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setStep(1)}>Back</Button>
            <Button onClick={handlePhoneSubmit} disabled={phone.trim().length !== 11}>Next</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isOpen && step === 3} onOpenChange={(open) => !open && handleClose()}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Please provide the full and accurate address for delivery.</DialogDescription>
          </DialogHeader>
