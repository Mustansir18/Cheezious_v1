...
const KitchenItemSlip = ({ itemGroup, onTogglePrepared }: KitchenItemSlipProps) => {
  ...
  return (
    <Card
      className={cn(
        'mb-4 break-inside-avoid border-2 bg-card shadow-lg transition-all',
        isPrepared ? 'border-green-500/50 opacity-50' : 'border-primary/50'
      )}
    >
      <CardHeader className="p-3">
        <div className="flex justify-between items-start">
          <div>
            ...
            {itemGroup.dealName && (
              <CardDescription className="text-sm text-primary font-bold pt-1">
                Part of "{itemGroup.dealName}"
              </CardDescription>
            )}
          </div>
      ...
      <CardContent className="p-3 pt-0">
        <p className="font-bold text-4xl mb-2">{quantity}x</p>
...
const DispatchOrderSlip = ({ order, onDispatchItem }: DispatchOrderSlipProps) => {
  ...
  return (
    <Card className="flex flex-col h-full border-2 border-blue-500/50 shadow-lg">
      <CardHeader className="p-4">
        <div className="flex justify-between items-start">
          ...
          <Badge
            variant={order.orderType === 'Dine-In' ? 'secondary' : 'outline'}
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={cn(
                      ...
                      item.isDispatched ? 'bg-green-500' : item.isPrepared ? 'bg-yellow-500 animate-pulse' : 'bg-gray-400'
                    )}
                  ></span>
                  <span>
...
const StationStats = ({ items }: { items: OrderItem[] }) => {
    const { avgCompletionTime, maxCompletionTime, minCompletionTime } = useMemo(() => {
        const completedItems = items.filter(item => item.isPrepared && item.order?.orderDate && item.preparedAt);
        if (completedItems.length === 0) {
            return { avgCompletionTime: 0, maxCompletionTime: 0, minCompletionTime: 0 };
        }
        const completionTimes = completedItems.map(item => {
            const startTime = new Date(item.order.orderDate).getTime();
            const endTime = new Date(item.preparedAt!).getTime();
            return (endTime - startTime) / (1000 * 60); // in minutes
        });
        const totalMinutes = completionTimes.reduce((sum, time) => sum + time, 0);
        return {
            avgCompletionTime: totalMinutes / completionTimes.length,
            maxCompletionTime: Math.max(...completionTimes),
            minCompletionTime: Math.min(...completionTimes),
        };
    }, [items]);
    const stats = [
        { title: 'Avg Time', value: `${avgCompletionTime.toFixed(1)} min`, icon: Clock },
        { title: 'Min Time', value: `${minCompletionTime.toFixed(1)} min`, icon: TrendingDown },
        { title: 'Max Time', value: `${maxCompletionTime.toFixed(1)} min`, icon: TrendingUp },
    ];
    return (
...
const StationColumn = ({ stationId, orders, onTogglePrepared }: StationColumnProps) => {
  const { itemGroupsForStation, allItemsForStation } = useMemo(() => {
    const itemGroups: KDSItemGroup[] = [];
    let allItems: OrderItem[] = [];

    orders.forEach((order) => {
      const relevantItems = order.items.filter((item) => item.stationId === stationId);
      if(relevantItems.length > 0) {
        allItems = [...allItems, ...relevantItems.map(i => ({...i, order}))];
      }

      const unpreparedItems = relevantItems.filter(item => !item.isPrepared);
      ...
      unpreparedItems.forEach((item) => {
        ...
        (item.selectedAddons || []).forEach((addon) => {
          const addonKey = addon.name; // Group by name is sufficient here
    ...
    return {itemGroupsForStation: itemGroups, allItemsForStation: allItems};
  }, [orders, stationId]);

  return (
    <ScrollArea className="h-full">
      <StationStats items={allItemsForStation} />
...
const DispatchColumn = ({ orders, onDispatchItem }: DispatchColumnProps) => {
  const ordersForDispatch = useMemo(() => {
    return orders.filter((order) => {
      ...
      return (isInDispatchStatus || (order.status === 'Pending' && hasPreparedItems)) && hasPreparedItems;
    });
  }, [orders]);
        ...
        <TabsList className="grid w-full grid-cols-5 h-auto">
          {stationTabs.map((tab) => (
            <TabsTrigger
              key={tab.id}
              value={tab.id}
              className="py-3 text-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
            >
